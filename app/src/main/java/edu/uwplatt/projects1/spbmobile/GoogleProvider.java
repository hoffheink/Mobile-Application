...
public class GoogleProvider {
    ...
    void setGoogleAccount(GoogleSignInAccount account) {
        if (account != null) {
            ...
            CloudDatasource cloudDatasource = CloudDatasource.getInstance(ourContext, account);
            simpleStorageSystem.saveSubArn(ourContext, cloudDatasource);
            ...
            AsyncTaskResult response = lambdaPlatform.invokeLambdaFunction(
                    LambdaFunctionNames.SETUP_COGNITO_SYNC, gson.toJson(cognitoSyncFormat),
                    cloudDatasource.getCognitoCachingCredentialsProvider());
            Log.d("CognitoSync: ", response.getResult().toString());
        }
